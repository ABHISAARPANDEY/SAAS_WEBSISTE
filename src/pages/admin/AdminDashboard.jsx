      ...
      <div className="min-h-screen bg-dark-primary">
        ...
        {isModalOpen && selectedQuote && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            ...
            >
              <div className="p-6 border-b border-gray-200 flex items-center justify-between bg-accent-primary">
                ...
                <button
                  ...
                  className="text-white/80 hover:text-white p-2 hover:bg-accent-secondary/50 rounded-full transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-8 bg-white">
                {/* Client Information */}
                <div>
                  <h3 className="text-lg font-semibold text-text-primary mb-4 font-orbitron flex items-center gap-2">
                    <Users className="w-5 h-5 text-accent-primary" />
                    Client Information
                  </h3>
                  <div className="bg-secondary rounded-xl p-6 border border-border-color hover:border-accent-primary/30 transition-colors shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-accent-primary mb-1">
                          <User className="w-4 h-4" />
                          <p className="text-text-secondary text-sm font-medium">Full Name</p>
                        </div>
                        <p className="text-text-primary font-medium pl-6">{selectedQuote.full_name || selectedQuote.contactName || selectedQuote.clientName || 'N/A'}</p>
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-accent-primary mb-1">
                          <Building2 className="w-4 h-4" />
                          <p className="text-text-secondary text-sm font-medium">Company</p>
                        </div>
                        <p className="text-text-primary font-medium pl-6">{selectedQuote.company_name || selectedQuote.company || 'Not provided'}</p>
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-accent-primary mb-1">
                        ...
                        <p className="text-text-primary font-medium pl-6">{selectedQuote.email}</p>
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-accent-primary mb-1">
                          <Phone className="w-4 h-4" />
                          <p className="text-text-secondary text-sm font-medium">Phone</p>
                        </div>
                        <p className="text-text-primary font-medium pl-6">{selectedQuote.phone_number || selectedQuote.phone || 'N/A'}</p>
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-accent-primary mb-1">
                          <Calendar className="w-4 h-4" />
                          <p className="text-text-secondary text-sm font-medium">Submitted On</p>
                        </div>
                        <p className="text-text-primary font-medium pl-6">{formatDate(selectedQuote.created_at || selectedQuote.timestamp)}</p>
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-accent-primary mb-1">
                          <Tag className="w-4 h-4" />
                          <p className="text-text-secondary text-sm font-medium">Reference ID</p>
                        </div>
                        <p className="text-text-primary font-medium pl-6">{selectedQuote.id.substring(0, 8).toUpperCase()}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-secondary rounded-xl p-6 border border-border-color hover:border-accent-secondary/30 transition-colors shadow-sm">
                    <div className="space-y-4">
                      <div className="bg-white rounded-lg p-4 border border-border-color">
                        <p className="text-text-primary whitespace-pre-line leading-relaxed">{selectedQuote.requirements || selectedQuote.projectDescription || 'No requirements provided'}</p>
                      </div>
                      ...
                      {selectedQuote.notes && (
                        <div>
                          <div className="flex items-center gap-2 text-accent-primary mb-2">
                            ...
                            <p className="text-text-secondary text-sm font-medium">Additional Notes</p>
                          </div>
                          <div className="bg-white rounded-lg p-4 border border-border-color">
                            <p className="text-text-primary whitespace-pre-line leading-relaxed">{selectedQuote.notes}</p>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                <div>
                  ...
                  <div className="bg-secondary rounded-xl p-6 border border-border-color hover:border-accent-tertiary/30 transition-colors shadow-sm">
                    <div className="flex flex-col md:flex-row gap-4">
                      <div className="flex-1">
                        <p className="text-text-secondary text-sm mb-2 font-medium">Current Status</p>
                        <span className={`px-3 py-1.5 inline-flex text-sm font-semibold rounded-full shadow-sm ${getStatusColor(selectedQuote.status)}`}>
                          {selectedQuote.status || 'pending'}
                        </span>
                      </div>
                      
                      <div className="flex-1">
                        <p className="text-text-secondary text-sm mb-2 font-medium">Update Status</p>
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors ${
                              selectedQuote.status === 'pending'
                                ? 'bg-white text-yellow-500 border border-yellow-500 shadow-sm'
                                : 'bg-white text-text-secondary border border-border-color hover:border-yellow-500 hover:text-yellow-500'
                            }`}
                          >
                            Pending
                          </button>
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors ${
                              selectedQuote.status === 'contacted'
                                ? 'bg-white text-blue-500 border border-blue-500 shadow-sm'
                                : 'bg-white text-text-secondary border border-border-color hover:border-blue-500 hover:text-blue-500'
                            }`}
                          >
                            Contacted
                          </button>
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors ${
                              selectedQuote.status === 'in-progress'
                                ? 'bg-white text-purple-500 border border-purple-500 shadow-sm'
                                : 'bg-white text-text-secondary border border-border-color hover:border-purple-500 hover:text-purple-500'
                            }`}
                          >
                            In Progress
                          </button>
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors ${
                              selectedQuote.status === 'completed'
                                ? 'bg-white text-green-500 border border-green-500 shadow-sm'
                                : 'bg-white text-text-secondary border border-border-color hover:border-green-500 hover:text-green-500'
                            }`}
                          >
                            Completed
                          </button>
                ...
                <div className="flex justify-between items-center pt-6 border-t border-border-color">
                  <div>
                    <button
                      ...
                      className="px-4 py-2 bg-error/10 text-error border border-error/30 rounded-lg hover:bg-error/20 transition-colors flex items-center gap-2"
                    >
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={closeQuoteModal}
                      className="px-4 py-2 bg-secondary text-text-secondary border border-border-color rounded-lg hover:bg-tertiary transition-colors flex items-center gap-2"
                    >
                      <X className="w-4 h-4" />
                      Close
                    </button>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-accent-primary/10 text-accent-primary border border-accent-primary/30 rounded-lg hover:bg-accent-primary/20 transition-colors flex items-center gap-2 hover:shadow-button"
                    >
